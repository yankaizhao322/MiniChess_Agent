...
# # EPS_END = 0.05
# # EPS_DECAY = 10000 
# # LR = 1e-4        # 【修改1】降低学习率，防止震荡
# # TARGET_UPDATE = 20
# # MEMORY_SIZE = 50000
# # NUM_EPISODES = 30000 

# # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# # print(f"🚀 Training Device: {device} | Mode: Stable Defense")
...
# #                 done = True
# #             else:
# #                 reward = (score_end - score_start) * 20 
# #                 reward -= 1 
# #                 next_state_tensor = board_to_tensor(board, device)

# #             memory.append((state_tensor, action_tensor := torch.tensor([[action_idx]], device=device), 
# #                            reward_tensor := torch.tensor([reward], device=device), next_state_tensor, legal_moves))
...
# #         if i_episode % 500 == 0:
# #             avg = np.mean(stats["rewards"][-100:]) if len(stats["rewards"]) > 100 else total_reward
# #             print(f"Ep {i_episode} | Avg Reward: {avg:.1f} | Eps: {eps_threshold:.2f}")
...
# GAMMA = 0.90        # 稍微短视一点，专注眼前的吃子和胜利
# EPS_START = 1.0
# EPS_END = 0.05
# EPS_DECAY = 2000    # 【关键】2000局内就把探索降到底，逼它快速学会
# LR = 1e-3           # 【关键】高学习率，学得快
# TARGET_UPDATE = 10
# MEMORY_SIZE = 10000
# NUM_EPISODES = 15000 
