...
#     """目的格如果有敌子，给一个小 bonus，用来更积极换子减少 draw"""
...
#     """
#     目标：
#     1) 永远走合法步（否则直接判负） :contentReference[oaicite:1]{index=1}
#     2) self-play 尽量少 draw：用“换子偏好 + anti-repetition + 少量随机”
#     """
#     def __init__(self):
#         self.device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")

#         # env 只创建一次！否则每步都开 fig 会内存爆炸
...
#         # anti-repetition: 记住最近的局面 hash（canonical）
#         self.recent = deque(maxlen=24)

#         # small randomness to break self-play loops
#         self.eps_play = 0.03
#         self.topk = 5

#     def get_action(self, board, player):
#         # 1) legal moves (必须从 env 获得)
...
#         # 3) epsilon randomness (very small)
...
#             # 5) add small capture bonus + repetition penalty (in score space)
#             #    用 CPU 算，不影响 torch
...
#             # 6) pick among top-k (tie-breaking randomness reduces self-play draw a lot)
